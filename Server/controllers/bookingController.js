...
import { lateCheckoutFee, prettyTime, cancellationLine, POLICY_DEFAULTS } from "../utils/stayPolicy.js";
 ...
 *   room charges -> tax on room charges -> meal inclusions (tax inclusive)
 *   -> extras -> discount -> grand total
 */
function priceBooking({ rooms, nights, adults, children, mealPlan, taxPct, extras, discount }) {
  const room_charges = rooms.reduce((s, r) => s + num(r.rate_per_night) * nights, 0);
  const meal_charges = mealPlan
    ? (num(adults) * num(mealPlan.supplement_per_adult) +
       num(children) * num(mealPlan.supplement_per_child)) * nights
    : 0;
  const tax_amount  = room_charges * (num(taxPct, 10) / 100);
  const grand_total = room_charges + tax_amount + meal_charges + num(extras) - num(discount);
  return {
    room_charges: +room_charges.toFixed(2),
    meal_charges: +meal_charges.toFixed(2),
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT r.room_id, r.room_number, r.floor, r.hk_status,
              ...
              rt.base_occupancy, rt.max_occupancy, rt.description, rt.amenities, rt.images,
              NOT EXISTS (
    ...
    rows.forEach(r => {
      const t = (byType[r.room_type_id] ||= {
        ...
        base_rate: r.base_rate, base_occupancy: r.base_occupancy, max_occupancy: r.max_occupancy,
...
export async function createBooking(req, res, next) {
  ...
  try {
    ...
    const taxPct       = pctField(tax_pct, "Tax %", 10);
    ...
    // Resolve meal plan for pricing
    let mealPlan = null;
    if (meal_plan_id) {
      const mp = await client.query('SELECT * FROM "MEAL_PLAN" WHERE plan_id = $1', [Number(meal_plan_id)]);
      mealPlan = mp.rows[0] || null;
    }
    ...
    for (const r of rooms) {
      const rt = await client.query(
        'SELECT room_type_id, base_rate, max_occupancy FROM "ROOM_TYPE" WHERE room_type_id = $1',
      ...
      capacity += num(rt.rows[0].max_occupancy);
      pricedRooms.push({
        room_id: r.room_id ? Number(r.room_id) : null,
        room_type_id: rt.rows[0].room_type_id,
        rate_per_night: r.rate_per_night != null && r.rate_per_night !== ""
          ? moneyField(r.rate_per_night, "Rate per night")
          : num(rt.rows[0].base_rate),
        adults: countField(r.adults, "Adults", { min: 0, max: 99, dflt: 1 }),
        children: countField(r.children, "Children", { min: 0, max: 99, dflt: 0 }),
      });
    }
    ...
    const bk = await client.query(
      `INSERT INTO "BOOKING"
        (booking_ref, b_id, guest_id, agent_id, source, check_in_date, check_out_date, nights,
         ...
         grand_total, advance_paid, promotion, remarks, taken_by)
    ...
    if (advanceAmt > 0) {
      await client.query(
        ...
        [booking.booking_id, advanceAmt, advanceMethod, req.user?.u_id || null]
      );
    }
...
export async function updateBooking(req, res, next) {
  ...
  try {
    ...
    const checkIn  = req.body.check_in_date  ? dateField(req.body.check_in_date,  "Check-in date")  : b.check_in_date.toISOString().slice(0, 10);
    const checkOut = req.body.check_out_date ? dateField(req.body.check_out_date, "Check-out date") : b.check_out_date.toISOString().slice(0, 10);
    const nights   = nightsBetween(checkIn, checkOut);
    if (nights < 1) {
      await client.query("ROLLBACK");
    ...
    const taxPct   = req.body.tax_pct  != null ? pctField(req.body.tax_pct, "Tax %", 10)                                 : num(b.tax_pct, 10);
    ...
    const mealPlanId = req.body.meal_plan_id !== undefined
      ? (req.body.meal_plan_id ? Number(req.body.meal_plan_id) : null)
      : b.meal_plan_id;

    let mealPlan = null;
    if (mealPlanId) {
      const mp = await client.query('SELECT * FROM "MEAL_PLAN" WHERE plan_id = $1', [mealPlanId]);
      mealPlan = mp.rows[0] || null;
    }
    ...
    if (Array.isArray(req.body.rooms) && req.body.rooms.length) {
      ...
      for (const r of req.body.rooms) {
        const rt = await client.query('SELECT room_type_id, base_rate, max_occupancy FROM "ROOM_TYPE" WHERE room_type_id = $1', [Number(r.room_type_id)]);
        if (!rt.rows.length) {
          await client.query("ROLLBACK");
          res.status(400); return next(new Error(`Unknown room_type_id ${r.room_type_id}`));
        }
        ...
        pricedRooms.push({
          room_id: r.room_id ? Number(r.room_id) : null,
          room_type_id: rt.rows[0].room_type_id,
          rate_per_night: r.rate_per_night != null && r.rate_per_night !== "" ? moneyField(r.rate_per_night, "Rate per night") : num(rt.rows[0].base_rate),
          adults: countField(r.adults, "Adults", { min: 0, max: 99, dflt: 1 }),
          children: countField(r.children, "Children", { min: 0, max: 99, dflt: 0 }),
        });
      }
      ...
      const existing = await client.query(
        `SELECT br.*, rt.max_occupancy FROM "BOOKING_ROOM" br
         JOIN "ROOM_TYPE" rt ON rt.room_type_id = br.room_type_id
         WHERE br.booking_id = $1`, [id]
      );
      pricedRooms = existing.rows;
      capacity = existing.rows.reduce((sum, r) => sum + num(r.max_occupancy), 0);
    }
    assertOccupancy(adults, children, capacity, pricedRooms.length);

    const totals = priceBooking({ rooms: pricedRooms, nights, adults, children, mealPlan, taxPct, extras, discount });
    ...
    const { rows } = await client.query(
      `UPDATE "BOOKING" SET
         ...
         meal_plan_id=$6, tax_pct=$7, extra_charges=$8, discount=$9,
         room_charges=$10, meal_charges=$11, tax_amount=$12, grand_total=$13,
         agent_id      = COALESCE($14, agent_id),
         source        = COALESCE($15, source),
         arrival_time  = COALESCE($16, arrival_time),
         special_requests = COALESCE($17, special_requests),
         promotion     = COALESCE($18, promotion),
         remarks       = COALESCE($19, remarks),
         status        = COALESCE($20, status)
       WHERE booking_id = $21 RETURNING *`,
      [checkIn, checkOut, nights, adults, children, mealPlanId, taxPct, extras, discount,
       totals.room_charges, totals.meal_charges, totals.tax_amount, totals.grand_total,
       req.body.agent_id ? Number(req.body.agent_id) : null,
       oneOf(req.body.source, "Source", BOOKING_SOURCES),
       timeField(req.body.arrival_time, "Arrival time"),
       textField(req.body.special_requests, "Special request", 2000),
       textField(req.body.promotion, "Promotion", 100),
       textField(req.body.remarks, "Remarks", 2000),
       oneOf(req.body.status, "Status", BOOKING_STATUSES), id]
    );
...
export async function checkIn(req, res, next) {
  ...
  try {
    ...
    const brs = await client.query(
      `SELECT br.*, r.room_number, rt.type_name
       FROM "BOOKING_ROOM" br
       LEFT JOIN "ROOM" r ON r.room_id = br.room_id
       JOIN "ROOM_TYPE" rt ON rt.room_type_id = br.room_type_id
    ...
    if (num(b.meal_charges) > 0) {
      const mp = await client.query('SELECT plan_name FROM "MEAL_PLAN" WHERE plan_id=$1', [b.meal_plan_id]);
      await post("meal", `${mp.rows[0]?.plan_name || "Meal plan"} — ${b.adults} adult(s), ${b.children} child(ren) × ${b.nights} night(s)`,
                 1, b.meal_charges, b.meal_charges);
    }
...
export async function addPayment(req, res, next) {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO "BOOKING_PAYMENT" (booking_id, amount, method, kind, reference, received_by)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [id, paid, payMethod, payKind, payRef, req.user?.u_id || null]
    );
  ...
  try {
    ...
    if (num(req.body?.settle_amount) > 0) {
      await client.query(
        ...
        [id, num(req.body.settle_amount), req.body.method || "cash", req.user?.u_id || null]
      );
    }
...
export async function getConfirmation(req, res, next) {
  try {
    ...
    const inTime  = prettyTime(policy.check_in_time);
    const outTime = prettyTime(policy.check_out_time);
    ...
    const text = [
      ...
      `*Check-In:* ${dmy(b.check_in_date)} (from ${inTime})`,
      `*Check-Out:* ${dmy(b.check_out_date)} (by ${outTime})`,
      `*Nights:* ${b.nights}`,
    ...
    res.json({
      booking: b,
      hotel,
      policy: { ...policy, check_in_pretty: inTime, check_out_pretty: outTime,
                cancellation_line: cancellationLine(policy) },
    ...
    const { rows } = await pool.query(
      `INSERT INTO "HOTEL_POLICY"
         (b_id, check_in_time, check_out_time, late_grace_minutes, late_mode,
          ...
          cancel_free_hours, cancel_charge_nights, updated_at)
       ...
       ON CONFLICT (b_id) DO UPDATE SET
         ...
         cancel_free_hours=$9, cancel_charge_nights=$10, updated_at=NOW()
       RETURNING *`,
      [b_id, checkIn, checkOut, grace, mode, pct, flat, cutoff, freeHrs, cancelN]
    );
